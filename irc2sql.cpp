 ...
 public:
	...
	void OnError(const SQL::Result &r) anope_override
	{
		if (!r.GetQuery().query.empty())
			Log(LOG_DEBUG) << "m_stats: Error executing query " << r.finished_query << ": " << r.GetError();
		else
			Log(LOG_DEBUG) << "m_stats: Error executing query: " << r.GetError();
	}
};
...
class M_irc2sql : public Module
{
 ...
 public:
	...
	void OnReload(Configuration::Conf *config) anope_override
	{
		...
		prefix = block->Get<const Anope::string>("prefix", "anope_stats_");

		Anope::string engine = block->Get<const Anope::string>("engine");
		this->sql = ServiceReference<SQL::Provider>("SQLProvider", engine);
		if (sql)
			this->CheckTables();
		else
	...
	{
		query = "INSERT DELAYED INTO `anope_stats_servers` (name, hops, comment, link_time, online, ulined) "
	...
	{
		query = "INSERT DELAYED INTO `anope_stats_servers` (name, online, split_time)"
